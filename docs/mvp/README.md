...
已收到 AW FLEX 系列资料；已确定淘宝中国中文母版及 Amazon 美国站英文 A+ 商品详情内容，初期采用 Basic A+ 制作范围。具体 SKU 适用范围、预期事实、部分平台规则及人工签收仍待补齐，详见 [业务输入清单](business-inputs.md)。[规则核对记录](platform-rules.md)与[模型试验方案](model-evaluation-proposal.md)已整理；未授权真实模型调用。M1 工程验收已完成；M0、M2 及最终业务验收仍保持开放。不依赖缺失输入的开发继续推进。
