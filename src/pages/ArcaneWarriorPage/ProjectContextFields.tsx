...
export function ContextControls({ context: c, session: s }: Props) {
  return <>
    ...
    {!!c.state?.versions.length && <>
      <label>复制已有配置<select value={String(c.selectedCopyVersion?.version ?? '')} disabled={!s.canWrite} onChange={e => c.setCopyVersion(e.target.value)}>{c.state.versions.map(version => <option key={version.version} value={version.version}>{version.label} · {version.context.productBrief.productName}{version.version === c.state?.activeVersion ? ' · 当前启用' : ''}</option>)}</select></label>
      <Button disabled={!s.canWrite || !c.selectedCopyVersion} onClick={() => c.selectedCopyVersion && c.requestCopy(c.selectedCopyVersion)}>复制 {c.selectedCopyVersion?.label} 为本地草稿</Button>
    </>}
    {c.pendingCopy && <div className="wide inspector-block" role="alert"><b>替换正在编辑的配置</b><p>将用 {c.pendingCopy.label} 替换当前本地输入；服务端草稿只在下一次明确保存时替换。</p><Button disabled={!s.canWrite} onClick={c.confirmCopy}>放弃本地输入并复制</Button><Button onClick={c.cancelCopy}>保留当前输入</Button></div>}
...
export function TargetFields({ context: c, session: s }: Props) {
  ...
  return <><div className="form-grid"><TextField context={c} field="platform" label="首发平台" /><TextField context={c} field="site" label="站点" />
    ...
    {s.catalog?.length === 0 && <p className="hint">暂无已核验的平台规则。可先保存草稿，补齐规则后再启用。</p>}
    {c.rule && <div className="inspector-block"><b>{c.readOnly ? '当前版本冻结的规则' : '所选规则'}</b><p>{c.rule.target.platform} / {c.rule.target.site} / {c.rule.target.country} / {c.rule.target.language} / {c.rule.target.currency} / {c.rule.target.unitSystem === 'metric' ? '公制' : '英制'}</p><p>核验人：{c.rule.verifiedBy} · {new Date(c.rule.verifiedAt).toLocaleDateString('zh-CN')}</p><a className="source-link" href={c.rule.officialUrl.startsWith('https://') ? c.rule.officialUrl : undefined} target="_blank" rel="noreferrer">查看规则来源</a><Button disabled={!c.canEdit} onClick={c.applyRuleTarget}>按规则填写目标市场</Button></div>}
  </>
}
...
export function CanvasFields({ context: c }: { context: ProjectContextController }) {
  return <><div className="form-grid"><label>图片宽度（px）<input inputMode="numeric" value={c.form.widthPx} disabled={!c.canEdit} maxLength={5} list="context-allowed-widths" onChange={e => c.setField('widthPx', e.target.value)} placeholder="填写整数像素" /><datalist id="context-allowed-widths">{c.rule?.allowedWidthsPx.map(width => <option key={width} value={width} />)}</datalist></label><label>图片格式<select value={c.form.format} disabled={!c.canEdit} onChange={e => c.setField('format', e.target.value)}><option value="">选择格式</option><option value="png">PNG</option><option value="jpeg">JPEG</option><option value="webp">WebP</option></select></label></div>{c.rule ? <p className="hint">规则允许：{c.rule.allowedWidthsPx.join('、')} px；{c.rule.allowedFormats.map(format => format.toUpperCase()).join('、')}。</p> : <p className="hint">选择核验规则后可查看允许的宽度与格式。未选择规则时仍可保存草稿。</p>}</>
}
...
export function ContextReadiness({ context: c, session: s }: Props) {
  ...
  return <><PanelTitle eyebrow="LAUNCH CHECK" title="准备状态" action={<Chip tone={c.activeVersion ? 'green' : 'muted'}>{c.activeVersion?.label ?? '未启用'}</Chip>} />
    ...
    {c.initialized && !c.readOnly && <div className="check-result"><b>启用前待处理</b>{issues.length ? issues.map(issue => <span key={issue.field}>{issue.message}</span>) : <span>字段完整，目标与当前规则匹配。</span>}{c.local.active && <span>还有本地修改，请先保存草稿。</span>}{c.local.needsReview && <span>服务端配置已变化，请先复核差异。</span>}</div>}
